...
from evennia.prototypes.prototypes import PrototypeEvMore, _
import re
import typing
...
from world.utils.utils import check_typeclass_perm, wrap, wrap_para, spawn
...
class ObjManipCommand(COMMAND_DEFAULT_CLASS):
    ...
    def get_object_typeclass(
        ...
        **kwargs,
        ...
        found_typeclass = typeclass or self.default_typeclasses.get(
            obj_type, None)
...
class CmdSetObjAlias(COMMAND_DEFAULT_CLASS):
    ...
    locks = "cmd:perm(setobjalias) or perm(Beginner Builder)"
    ...
    def func(self):
        ...
        if self.rhs is None and "delete" not in self.switches:
            ...
            if aliases:
                ...
                caller.msg(f"No aliases exist for '{
                           obj.get_display_name(caller)}'.")
            return

        ### Check permissions ###
        if not (obj.access(caller, "control") or obj.access(caller, "edit")):
            err = f"You don't have permission to set attibutes on |C{obj}|n."
        ...
        if "delete" in self.switches:
            ...
            if existed:
                ...
                caller.msg(
                    "%s has no alias '%s'." % (
                        obj.get_display_name(caller), self.rhs)
                )
            return
...
class CmdCopy(ObjManipCommand):
    ...
    key = "@copy"
    locks = "cmd:perm(copy) or perm(Beginner Builder)"
    help_category = "Building"
    ...
    def func(self):
        ...
        if not self.rhs:
            ...
            if not (
                (
                    from_obj.access(caller, "control")
                    or from_obj.access(caller, "examine")
                )
                and not check_typeclass_perm(caller, from_obj)
            ):
                err = f"You don't have permission to copy |C{from_obj}|n."
                caller.msg(wrap(err, width=_WIDTH))
                return
            ...
            if not (
                ...
                and not check_typeclass_perm(caller, from_obj, check="create")
            ):
                err = f"You don't have permission to copy |C{from_obj}|n."
                print(wrap(err, width=_WIDTH))
                return
            ...
            for objdef in self.rhs_objs:
                ...
                if to_obj_location:
                    to_obj_location = caller.search(
                        to_obj_location, global_search=True)
                ...
                if copiedobj:
                    string = f"Copied {from_obj_name} to '{to_obj_name}' (aliases: {
                        to_obj_aliases
                    })."
                else:
                    string = f"There was an error copying {from_obj_name} to '{
                        to_obj_name
                    }'."
        # we are done, echo to user
        caller.msg(wrap(string, witdth=68))
...
class CmdCpAttr(ObjManipCommand):
    ...
    key = "@cpattr"
    switch_options = ("move",)
    locks = "cmd:perm(cpattr) or perm(Beginner Builder)"
    help_category = "Building"
    ...
    def check_from_attr(self, obj, attr, category=None, clear=False):
        ...
        """
        caller = self.caller
        attribute = obj.attributes.get(
            attr, category=category, return_obj=True, return_list=True
        )
        attribute = attribute[0] if len(attribute) > 0 else None
        if attribute is not None:
            if obj.access(caller, "examine") and not clear:
                return True
            elif clear and (
                obj.access(caller, "examine") or obj.access(caller, "control")
            ):
                if attribute.locks.check(caller, "attredit", default=True):
                    return True
                else:
                    err = (
                        f"|RPermission Error: You don't have permission to set"
                        f" or delete the |w'{attr}'|n attribute from"
                        f" |G'{obj.key}'|n."
                    )
                    caller.msg(wrap(err, width=_WIDTH))
                    return False
                ...
                err = (
                    f"|RPermission error:|n You need |R'examine'|n"
                    f" permissions to copy or move from |G'{obj.key}'."
                )
                caller.msg(wrap(err, width=_WIDTH))
                return False
        else:
            err = f"|RError: No such attribute (|w'{obj.key}/{attr}'|r).|n"
            caller.msg(wrap(err, width=_WIDTH))
        return False

    def check_to_attr(self, obj, attr, category=None, clear=False):
        """
        Hook for overriding on subclassed commands. Checks to make sure a
        caller can copy the attr to the object in question. If not, return a
        false value and the command will abort. An error message should be
        provided by this function.
        ...
        caller = self.caller
        attribute = obj.attributes.get(
            key=attr, category=category, return_obj=True, return_list=True
        )
        attribute = attribute[0] if len(attribute) > 0 else None
        if attribute is not None:
            if (
                obj.access(caller, "edit")
                and attribute.locks.check(caller, "attredit", default=True)
                or obj.access(caller, "control")
            ):
                return True
            else:
                err = (
                    f"|RPermission Error:|n You don't have permission to edit"
                    f" or delete the |w{attr}|n attribute on"
                    f" |G'{obj.key}'|n."
                )
                caller.msg(wrap(err, width=_WIDTH))
                return False

        elif obj.access(caller, "attrcreate"):
            return True
        else:
            text = (
                f"|RPermission Error:|n You need |R'attrcreate'|n permissions"
                f" to create a |w'{attr}'|n attribute on"
                f" |G'{obj.key}'|n."
            )
    ...
    def check_has_attr(self, obj, attr, category=None):
        ...
        if not obj.attributes.has(attr, category=category):
            self.msg(
                wrap(
                    f"{obj.name} doesn't have an attribute {attr}{
    ...
    def func(self):
        ...
        from_obj_category_str = f"[{
            from_obj_category}]" if from_obj_category else ""
        ...
        if not from_obj or not to_objs:
            caller.msg(
                wrap(
                    "You have to supply both source object and target(s).", width=_WIDTH
                )
            )
            return
        ...
        if (len(from_obj_attrs) != len(set(from_obj_attrs))) and clear:
            self.msg(
                wrap(
                    "|RCannot have duplicate source names when moving!|n", width=_WIDTH
                )
            )
            return
        ...
        for to_obj in to_objs:
            ...
            to_obj_category_str = f"[{
                to_obj_category}]" if to_obj_category else ""
            ...
            for inum, from_attr in enumerate(from_obj_attrs):
                ...
                if clear and not (from_obj == to_obj and from_attr == to_attr):
                    from_obj.attributes.remove(
                        from_attr, category=from_obj_category)
                    result.append(
                        f"\nMoved |G{from_obj.name}.{from_attr}{
                            from_obj_category_str
                        }|n -> |G{to_obj_name}.{to_attr}{
                            to_obj_category_str
                        }|n. (value:"
                        f" {repr(value)})"
                    )
                    ...
                    result.append(
                        f"\nCopied |G{from_obj.name}.{from_attr}{
                            from_obj_category_str
                        }|n -> |G{to_obj.name}.{to_attr}{
                            to_obj_category_str
                        }|n. (value:"
                        f" {repr(value)})"
                    )
        caller.msg(wrap("".join(result), width=_WIDTH))
...
class CmdMvAttr(ObjManipCommand):
    ...
    key = "@mvattr"
    switch_options = ("copy",)
    locks = "cmd:perm(mvattr) or perm(Beginner Builder)"
    help_category = "Building"
...
class CmdCreate(ObjManipCommand):
    ...
    key = "@create"
    switch_options = ("drop",)
    locks = "cmd:perm(create) or perm(Beginner Builder)"
    help_category = "Building"
    ...
    def func(self):
        ...
        for objdef in self.lhs_objs:
            ...
            if errors:
                self.msg(wrap(f"|R{errors}|n", width=_WIDTH))
            if not obj_typeclass:
                continue
            if not check_typeclass_perm(caller, obj_typeclass):
                try:
                    err = f"You need permission |R'{
                        obj_typeclass.class_perm['create']
                    }'|n"
                    err += f" to create |C'{objdef['option']}'|n."
                    caller.msg(wrap(err, width=_WIDTH))
                except AttributeError:
                    err = (
                        f"You don't have permission to create |C'{
                            objdef['option']}'|n."
                    )
                    caller.msg(wrap(err, width=_WIDTH))
                continue
            ...
            if aliases:
                string = (
                    f"You create a new |C'{obj.typename}'|n: |G{obj.name}|n"
                    f" (aliases: |M{', '.join(aliases)}|n)."
                )
            else:
                string = f"You create a new |C'{
                    obj.typename}'|n: |G{obj.name}|n."
        ...
        if string:
            caller.msg(wrap(string, width=_WIDTH))
...
class CmdDesc(COMMAND_DEFAULT_CLASS):
    ...
    key = "@desc"
    switch_options = ("edit",)
    locks = "cmd:perm(desc) or perm(Beginner Builder)"
    help_category = "Building"
    ...
    def edit_handler(self):
        ...
        attr = obj.attributes.get("desc", return_obj=True, return_list=True)
        attr = attr[0] if len(attr) > 0 else None
        if attr is not None:
            if not obj.access(self.caller, "control") or (
                not obj.access(self.caller, "edit")
                and not attr.locks.check(self.caller, "attredit", default=True)
            ):
                err = (
                    f"|RPermission Error:|n You don't have permissions"
                    f" to edit the description of |G'{obj.name}'|n."
                )
                self.caller.msg(wrap(err, width=_WIDTH))
                return
        else:
            if not obj.access(self.caller, "attrcreate"):
                err = (
                    f"|RPermission Error:|n You don't have permissions"
                    f" to create the description of |G'{obj.name}'|n."
                )
                self.caller.msg(wrap(err, width=_WIDTH))
                return
    ...
    def func(self):
        ...
        # Check permissions
        if attr is not None:
            if obj.access(self.caller, "control") or (
                obj.access(self.caller, "edit")
                and attr.locks.check(self.caller, "attredit", default=True)
            ):
                obj.db.desc = desc
                text = (
                    f"The description was set on |G'{
                        obj.get_display_name(caller)}'|n."
                )
            else:
                text = (
                    f"|RPermission Error:|n You don't have permissions"
            ...
            self.caller.msg(wrap(text, width=_WIDTH))
            return
        else:
            obj.db.desc = desc
            text = f"The description was set on |G'{
                obj.get_display_name(caller)}'|n."
        self.caller.msg(wrap(text, width=_WIDTH))
...
class CmdDestroy(COMMAND_DEFAULT_CLASS):
    ...
    objects inside the object instead of moving them to their home location.
    Keep that in mind!
    """
    ...
    locks = "cmd:perm(destroy) or perm(Beginner Builder)"
    help_category = "Building"
    ...
    def func(self):
        ...
        def delobj(obj):
            ...
            string = ""
            if not obj.pk:
                string = f"Object |G'{obj.db_key}'|n was already deleted."
            else:
                objname = obj.name
                # if not ((obj.access(caller, "control") or obj.access(caller, "delete"))
                #    and check_typeclass_perm(caller, obj)):
                #    return f"\nYou don't have permission to delete {objname}."
                if obj.account and "override" not in self.switches:
                    return (
                        f"Object |G'{
                            objname
                        }'|n is controlled by an active account. Use /override to"
                        " delete anyway."
                    )
                if obj.dbid == int(settings.DEFAULT_HOME.lstrip("#")):
                    return (
                        f"|/You are trying to delete |G'{
                            objname
                        }'|n, which is set as DEFAULT_HOME. "
                ...
                # do the deletion
                okay = obj.delete(caller=caller)
                if not okay:
                    string += f"|G'{
                        objname
                    }'|n not destroyed, probably because because you don't have"
                    string += " permission. "
                else:
                    string += f"|G'{objname}'|n was destroyed. "
                    if had_exits:
                        string += f"Exits to and from |G'{
                            objname
                        }'|n were destroyed as well. "
                    # This is changed because objects inside obj are deleted aswell.
                    if had_objs:
                        string += f"Objects inside |G'{
                            objname
                        }'|n are also destroyed or moved"
                        string += f" back to their home locations."
            return string
        ...
        for objname in self.lhslist:
            ...
            if obj is None:
                self.msg(
                    wrap(
                        "|R(Objects to destroy must either be local or specified with a unique #dbref.)|n",
                        width=_WIDTH,
                    )
                )
        ...
        if objs and ("force" not in self.switches and type(self).confirm):
            ...
            if len(objs) == 1:
                confirm += f"|G'{objs[0].get_display_name(caller)}'|n"
            elif len(objs) < 5:
                confirm += ", ".join(
                    [f"|G'{obj.get_display_name(caller)}'|n" for obj in objs]
                )
            else:
                confirm += ", ".join(["|G'#{}'|n".format(obj.id)
                                     for obj in objs])
            confirm += "|/[yes]/no?" if self.default_confirm == "yes" else "|/yes/[no]"
            answer = ""
            answer = yield (wrap(confirm, width=_WIDTH))
            answer = self.default_confirm if answer == "" else answer

            if answer and answer not in ("yes", "y", "no", "n"):
                caller.msg(
                    wrap(
                        "|RCanceled|n: Either accept the default by pressing return or specify yes/no.",
                        width=_WIDTH,
                    )
                )
                delete = False
            elif answer.strip().lower() in ("n", "no"):
                caller.msg("|RCanceled|n: No object was destroyed.")
                delete = False
        ...
        if delete:
            ...
            if results:
                caller.msg(wrap("".join(results).strip(), width=_WIDTH))
...
class CmdDig(ObjManipCommand):
    ...
    locks = "cmd:perm(dig) or perm(Beginner Builder)"
    help_category = "Building"
    ...
    def func(self):
        ...
        if not check_typeclass_perm(caller, room_typeclass):
            try:
                err = f"|/|/You need class permission |R'{
                    room_typeclass.class_perm['create']
                }'|n"
                err += f" to create |G'{room['option']}'|n."
                caller.msg(wrap(err, width=_WIDTH))
            except AttributeError:
                err = f"|/|/You don't have class permission to create |G'{
                    room['optiom']
                }|n."
                caller.msg(wrap(err, width=_WIDTH))
            return
        ...
        room_string = f"Created room |G'{new_room}({new_room.dbref}){
            alias_string}'|n"
        room_string += f" of type |C{new_room.typeclass_path}|n."
        ...
        if self.rhs_objs:
            ...
            if not to_exit["name"]:
                exit_to_string = "|/No exit created to new room."
            elif not location:
                exit_to_string = "|/You cannot create an exit from a None-location."
            else:
                ...
                if errors:
                    self.msg(
                        wrap("|rError creating exit:|n |G'%s'|n" %
                             errors, width=_WIDTH)
                    )
                ...
                # Check Permissions
                if (
                    check_typeclass_perm(caller, exit_typeclass)
                    and location.access(caller, "edit")
                    and new_room.access(caller, "edit")
                ):
                    ...
                    if errors:
                        self.msg(
                            wrap(
                                "|rError creating exit:|n |G'%s'|n" % errors,
                                width=_WIDTH,
                            )
                        )
                    if new_to_exit:
                        ...
                        exit_to_string = (
                            f"\nCreated Exit from {
                                location.name} to {new_room.name}:"
                            f" {new_to_exit}({new_to_exit.dbref}){
                                alias_string}."
                        )
                else:
                    try:
                        err = (
                            f"|/|/You need |R'{
                                exit_typeclass.class_perm['create']}'|n"
                        )
                        err += f" permission to create |C'{
                            exit_typeclass}'|n exit and"
                        err += f" |R'edit'|n permissions on location |G'{
                            location}|n"
                        err += f" or destination |G'{
                            new_room}'|n to attach it."
                    except KeyError:
                        err = f"|/|/You don't have permissions to create |C'{
                            exit_typeclass
                        }'|n exit"
                        err += f" or have no |R'edit'|n permissions on location |G'{
                            location
                        }'|n."
                        err += f" or destination |G'{new_room}'|n."
                    self.msg(wrap(err, width=_WIDTH))
        ...
        if len(self.rhs_objs) > 1:
            ...
            if not back_exit["name"]:
                exit_back_string = "|/No back exit created."
            elif not location:
                exit_back_string = (
                ...
                if errors:
                    self.msg(wrap("|RError creating exit:|n %s" %
                             errors, width=_WIDTH))
                if exit_typeclass:
                    if (
                        check_typeclass_perm(caller, exit_typeclass)
                        ...
                        exit_back_string = (
                            f" Created Exit back from {new_room.name} to {
                                location.name
                            }:"
                            f" {new_back_exit}({new_back_exit.dbref}){
                                alias_string}."
                        )
                    else:
                        try:
                            err = f"|/|/You need |R'{
                                exit_typeclass.class_perm['create']
                            }'|n"
                            err += (
                                f" permission to create |C'{
                                    exit_typeclass}'|n exit and"
                            )
                            err += (
                                f" |R'edit'|n permissions on location |G'{
                                    new_room}|n"
                            )
                            err += f" or destination |G'{
                                location}'|n to attach it."
                        except KeyError:
                            err = f"|/|/You don't have permissions to create |C'{
                                exit_typeclass
                            }'|n exit"
                            ...
                            err += f" or destination |G'{location}'|n."
                        self.msg(wrap(err, width=_WIDTH))

        text = f"{room_string}{exit_to_string}{exit_back_string}"
        caller.msg(wrap(text, width=_WIDTH))
...
class CmdTunnel(COMMAND_DEFAULT_CLASS):
    ...
    locks = "cmd: perm(tunnel) or perm(Beginner Builder)"
    help_category = "Building"
    ...
    def func(self):
        ...
        if exitshort not in self.directions:
            string = (
                "tunnel can only understand the following directions: |C'%s'|n."
                % ",".join(sorted(self.directions.keys()))
            )
            string += "\n(use dig for more freedom)"
            self.msg(wrap(string, width=_WIDTH))
            return
...
class CmdLink(COMMAND_DEFAULT_CLASS):
    ...
    key = "@link"
    locks = "cmd:perm(link) or perm(Beginner Builder)"
    help_category = "Building"
    ...
    def func(self):
        ...
        if self.rhs:
            ...
            string = ""
            note = (
                "Note: |G'%s'|n(%s) did not have a destination set before. Make sure you link the right"
                " thing. "
            )
            ...
            if "twoway" in self.switches:
                if not (target.location and obj.location):
                    string = f"To create a two-way link, |G{obj}|n and |G{
                        target
                    }|n must both have a location"
                    string += " (i.e. they cannot be rooms, but should be exits)."
                if not target.destination:
                    string += note % (target.name, target.dbref)
                if obj.access(caller, "edit") and target.access(caller, "edit"):
                    obj.destination = target.location
                    target.destination = obj.location
                    string += (
                        f"|/|/Link created |G{obj.name}|n (in |G{
                            obj.location
                        }|n) <-> |G{target.name}|n (in"
                        f" |G{target.location}|n) (two-way)."
                    )
                else:
                    string += "|/|/|rPermission Error:|n You need |R'edit'|n"
                    string += " permissions on both exit objects to do that!"

            else:
                if obj.access(caller, "edit") and target.access(caller, "edit"):
                    obj.destination = target
                    string += f"|/|/Link created |G'{obj.name}'|n -> |G'{
                        target
                    }'|n (one way)."
                else:
                    string += "|/|/|rPermission Error:|n You need |R'edit'|n"
                    string += " permissions on exit |G'{obj.name}'|n to do that!"
            ...
            if dest:
                string = f"|G{obj.name}|n is an exit to |G{dest.name}|n."
            else:
                string = f"|G{obj.name}|n is |Rnot|n an exit. Its home location is |G{
                    obj.home
                }|n."
            ...
            if obj.access(caller, "edit"):
                if obj.destination:
                    ...
                    string = f"Former exit |G{
                        obj.name}|n no longer links anywhere."
                else:
                    string = f"|G{obj.name}|n had no destination to unlink."
            else:
                string = f"|RPermission Error:|n You need |R'edit'|n"
                string += f" permissions to change the destination of"
                string += f" |G{obj.name}|n."
        # give feedback
        caller.msg(wrap(string.strip(), width=_WIDTH))
...
class CmdUnLink(CmdLink):
    ...
    key = "unlink"
    locks = "cmd:perm(unlink) or perm(Beginner Builder)"
    help_key = "Building"
...
class CmdSetHome(CmdLink):
    ...
    key = "@sethome"
    locks = "cmd:perm(sethome) or perm(Beginner Builder)"
    help_category = "Building"
    ...
    def func(self):
        ...
        if not self.rhs:
            ...
            if not home:
                ...
                string = f"|G{obj}|n's current home is |G{
                    home}|n({home.dbref})."
        else:
            # set a home location
            ...
            if not new_home:
                return
            if obj.access(self.caller, "edit"):
                old_home = obj.home
                obj.home = new_home
                if old_home:
                    string = (
                        f"Home location of |G{obj}|n was changed from |G{old_home}|n({
                            old_home.dbref
                        }) to"
                        f" |G{new_home}|n({new_home.dbref})."
                    )
                else:
                    string = f"Home location of |G{obj}|n was set to |G{new_home}|n({
                        new_home.dbref
                    })."
            else:
                string = (
                    f"|RPermission Error:|n You need |R'edit'|n"
                    f" permissions on |G{obj}|n to change its home!"
                )
        self.msg(wrap(string, width=_WIDTH))
...
class CmdListCmdSets(COMMAND_DEFAULT_CLASS):
    ...
    key = "@cmdsets"
    locks = "cmd:perm(listcmdsets) or perm(Beginner Builder)"
    help_category = "Building"
    ...
    def func(self):
        ...
        if obj.access(caller, "examine"):
            ...
            caller.msg(
                wrap(
                    f"|RPermissions Error:|n You need |R'examine'|n permisions"
                    f" to read the Commandsets on |G'{obj.name}'|n.",
                    width=_WIDTH,
                )
            )
...
class CmdName(ObjManipCommand):
    ...
    key = "@name"
    aliases = ["@rename"]
    locks = "cmd:perm(rename) or perm(Beginner Builder)"
    help_category = "Building"
    ...
    def func(self):
            ...
            if objname.startswith("*") and caller.account:
                ...
                if obj:
                    ...
                    if not obj.access(caller, "control"):
                        caller.msg(
                            wrap(
                                f"|RPermission Error:|n You need |R'control'|n rights on account |G'{
                                    obj
                                }|n"
                                f" to change its name.",
                                width=_WIDTH,
                            )
                        )
                        return
                    ...
                    caller.msg(
                        wrap(
                            f"Account's name changed to |G'{newname}'|n.", width=_WIDTH
                        )
                    )
                    return
        ...
        if not obj.access(caller, "control"):
            caller.msg(
                wrap(
                    f"|RPermission Error:|n You need |R'control'|n permissions to change"
                    f" the name of |G'{obj}'|n.",
                    width=_WIDTH,
                )
            )
            return
        ...
        caller.msg(
            wrap(f"Object's name changed to |G'{
                 newname}'{astring}|n.", width=_WIDTH)
        )
...
class CmdOpen(ObjManipCommand):
    ...
    key = "@open"
    locks = "cmd:perm(open) or perm(Beginner Builder)"
    help_category = "Building"
    ...
    def create_exit(
        ...
        if exit_obj:
            ...
            if not exit_obj.destination:
                ...
                caller.msg(
                    wrap(
                        f"|RError:|n |G'{
                            exit_name
                        }'|n already exists and is not an exit!"
            ...
            if old_destination:
                string = f"Exit |G'{exit_name}'|n already exists."
                if old_destination.id != destination.id:
                    # reroute the old exit.
                    if (
                        exit_obj.access(caller, "edit")
                        and exit_obj.destination(caller, "edit")
                        and exit_obj.location(caller, "edit")
                    ):
                        ...
                        string += (
                            f" Rerouted its old destination |G'{
                                old_destination.name
                            }'|n to"
                            f" |G'{destination.name}'|n and changed aliases."
                        )
                    else:
                        string += f"|/|/|RPermission Error: You need |R'edit'|n on exit |G'{
                            exit_obj.name
                        }'|n,"
                        string += (
                            f" |G'{destination.name}'|n and |G'{
                                location.name}'|n."
                        )
                else:
            ...
            if (
                check_typeclass_perm(caller, exit_typeclass)
                and location.access(caller, "edit")
                and destination.access(caller, "edit")
            ):
                ...
                if exit_obj:
                    ...
                    string = (
                        f"Created new Exit |G'{exit_name}'|n from |G'{
                            location.name
                        }'|n to"
                        f" |G'{destination.name}'|n{string}."
                    )
                else:
                    string = f"Error: Exit '{exit.name}' not created."
            else:
                string += f"|Permission Error: You need |R'typeclass'|n permissions on |G'{
                    exit_typeclass
                }'|n"
                string += f" and |R'edit'|n permissions on |G'{
                    location.name
                }'|n and |G'{destination.name}'|n."
        # emit results
        caller.msg(wrap(string, width=_WIDTH))
        return exit_obj
    ...
    def parse(self):
        ...
        if not self.location:
            self.msg(
                wrap("You cannot create an exit from a None-location.", width=_WIDTH)
            )
            raise InterruptCommand
...
def _convert_from_string(cmd, strobj):
    ...
    try:
        ...
        string = f'|RNote|n: name "|R{
            strobj
        }|n" was converted to a string. Make sure this is acceptable.'
        cmd.caller.msg(wrap(string, width=_WIDTH))
        return strobj
    except Exception as err:
        string = f"|RUnknown error in evaluating Attribute: '{err}'|n"
        return string
...
class CmdSetAttribute(ObjManipCommand):
    ...
    key = "@set"
    locks = "cmd:perm(set) or perm(Beginner Builder)"
    help_category = "Building"
    ...
    def check_obj(self, obj):
        ...
        if not (obj.access(self.caller, "control") or obj.access(self.caller, "edit")):
            self.caller.msg(
                wrap(
                    f"|RPermission Error:'|n You need |R'edit'|n"
                    f" or |R'control'|n permissions"
                    f" on |G'{obj.key}'|n to change it's attributes!",
                    width=_WIDTH,
                )
            )
            return False
    ...
    def check_attr(self, obj, attr_name, category):
        ...
        caller = self.caller
        attribute = obj.attributes.get(
            key=attr_name, category=category, return_obj=True, return_list=True
        )
        attribute = attribute[0] if len(attribute) > 0 else None
        ...
        if attribute is not None:
            if (
                ...
                err = (
                    f"|RPermission Error:|n You don't have permission to edit"
                    f" or delete the |w{attr_name}|n attribute."
                )
                caller.msg(wrap(err, width=_WIDTH))
                return None
        else:
            if obj.access(caller, "attrcreate"):
                return attr_name
            else:
                err = (
                    f"|RPermissions Error:|n You need 'attrcreate' permissions"
                    f" to create Attributes on |G'{obj}'|n."
                )
                caller.msg(wrap(err, width=_WIDTH))
                return None
        return None
    ...
    def view_attr(self, obj, attr, category):
        ...
        error = f"\nAttribute {
            obj.name}/|w{attr} [category:{category}] does not exist."
        if nested:
                ...
                if nested_keys:
                    ...
                    if deep is not self.not_found:
                        try:
                            ...
                            continue
                    return f"\nDeleted attribute |G{obj.name}|n/|w{attr}|n [category:{category}]."
                else:
                    ...
                    if exists:
                        ...
                        return (
                            f" No attribute |G{obj.name}|n/|w{attr}|n [category: {
                                category
                            }] "
                            "was found to delete."
                        )
        error = f" No attribute |G{obj.name}|n/|w{attr}|n [category: {
            category
        }] was found to delete."
        if nested:
            error += " (Nested lookups attempted)"
        ...
        try:
            if not done:
                if verb == "Created":
                    ...
                    obj.attributes.get(
                        attr, category=category, return_obj=True
                    ).locks.clear()
                else:
                    locks = str(
                        obj.attributes.get(
            ...
            text = (
                f" {verb} attribute |G{obj.name}|n/|w{attr}|n [category:{category}] = {
                    value
                } |R("
                f"{str(obj.attributes.get(attr, category, return_obj=True).locks)})|n"
            )
            return text
        except SyntaxError:
    ...
    def edit_handler(self, obj, attr, caller):
        ...
        try:
            ...
            if not isinstance(old_value, str):
                answer = yield (
                    wrap(
                        f"|RWarning: Attribute |w{attr}|R is of type |w{
    ...
    def search_for_obj(self, objname):
        ...
        if objname.startswith("*") or "account" in self.switches:
            ...
            found_obj = _AT_SEARCH_RESULT(
                search.search_script(objname), caller)
        elif "channel" in self.switches:
            found_obj = _AT_SEARCH_RESULT(
                search.search_channel(objname), caller)
        else:
            global_search = True
    ...
    def func(self):
        ...
        if "edit" in self.switches:
            ...
            if not attrs:
                ...
                return
            attr = self.check_attr(obj, attrs[0], category)
            if attr is not None:
        ...
        if not value:
            if self.rhs is None:
                if obj.access(caller, "examine"):
                    ...
                    caller.msg(
                        wrap(
                            f"|RPermission Error:'|n You need |R'examine'|n permissions"
                            f" on |G'{obj.key}'|n to inspect it's attributes!",
                            width=_WIDTH,
                        )
                    )
                    return
            ...
            for attr in attrs:
                ...
                if hasattr(parsed_value, "access"):
                    ...
                    if not (
                        ...
                        or parsed_value.access(caller, "edit")
                    ):
                        caller.msg(
                            wrap(
                                f"You don't have permission to set object with identifier '{
                                    value
                                }'.",
                                width=_WIDTH,
                            )
        ...
        if not result:
            ...
            # send feedback
            caller.msg(wrap("".join(result).strip("\n"), width=_WIDTH))
...
class CmdTypeclass(COMMAND_DEFAULT_CLASS):
    ...
    locks = "cmd:perm(typeclass) or perm(Beginner Builder)"
    help_category = "Building"
    ...
    def _generic_search(self, query, typeclass_path):
        ...
        if typeclass_path:
            ...
            if caller.__dbclass__ == dbclass:
                ...
                caller.msg(
                    wrap(
                        f"Trying to search |G'{new_typeclass}'|n with query '{
                            self.lhs
                        }'.",
                        width=_WIDTH,
                    )
                ...
                if not obj:
                    if isinstance(obj, list):
                        caller.msg(
                            wrap(
                                f"Could not find |G{new_typeclass}|n with query '{
                                    self.lhs
                                }'.",
                                width=_WIDTH,
                            )
    ...
    def func(self):
        ...
        if "list" in self.switches or self.cmdname in ("typeclasses", "@typeclasses"):
            tcls = get_all_typeclasses()
            tclasses = [
                t for t in tcls if check_typeclass_perm(caller, t, check="create")
            ]
            ...
            evmore.msg(caller, string, exit_on_lastpage=True)
        ...
        if "show" in self.switches or "examine" in self.switches:
            ...
            if not obj:
                ...
                matches = [
                    ...
                    if key.endswith(oquery) and check_typeclass_perm(caller, key)
                ]
                ...
                if nmatches > 1:
                    caller.msg(
                        wrap(
                            "Multiple typeclasses found matching {}:|/|/|G{}|n".format(
                                oquery, ",  ".join(tup[0] for tup in matches)
                            ),
                            width=_WIDTH,
                        )
                    )
                    ...
                    caller.msg(
                        wrap(
                            f"No object or typeclass path found to match |G'{
                                oquery
                            }'|n with your permissions.",
                            width=_WIDTH,
                        )
                    )
                else:
                    ...
                    evmore.msg(caller, doc_string, exit_on_lastpage=True)
            else:
                # do the search again to get the error handling in case of multi-match
                obj = caller.search(oquery)
                if not obj:
                    return
                if check_typeclass_perm(caller, obj):
                    caller.msg(
                        wrap(
                            f"|G{obj.name}'s|n current typeclass is"
                            f" '|G{obj.__class__.__module__}.{
                    ...
                    caller.msg(
                        wrap(
                            f"|/You don't have permissions to access typeclass of '|G{
                                obj.name
                            }|n'.",
                            width=_WIDTH,
                        )
        ...
        if "prototype" in self.switches:
            key = self.rhs
            pcls = protlib.search_prototype(key=key)
            prototype = [
                p for p in pcls if check_typeclass_perm(caller, p["typeclass"])
            ]
            if len(prototype) > 1:
                caller.msg(
                ...
                caller.msg(
                    wrap(
                        f"No prototype '{
                            key}' was found with your permissions.",
                        width=_WIDTH,
                    )
                )
                return
        ...
        if "show" in self.switches or "examine" in self.switches:
            if not obj.access(caller, "examine"):
                caller.msg(
                    wrap(
                        f"|RPermissions Error: You need |R'examine'|n permissions"
                        f" on |G'{obj.name}'|n to show it's typeclass!",
                        width=_WIDTH,
                    )
                )
                return
            caller.msg(
                wrap(
                    f"|G{obj.name}|n's current typeclass is |G'{
                        obj.__class__}'|n",
                    width=_WIDTH,
                )
            )
        ...
        if not obj.access(caller, "control") or not check_typeclass_perm(caller, obj):
            try:
                caller.msg(
                    wrap(
                        f"|RPermission Error:|n"
                        f" You need |R'{
                            obj.class_perm['create']
                        }'|n typeclass permissions and"
                        f" |R'control'|n"
                        f" prermissions to change |C'{obj.key}'|n.",
                        width=_WIDTH,
                    )
                )
            except KeyError:
                caller.msg(
                    wrap(
                        f"|RPermission Error:"
                        f" You don't have prermisssions to change |C'{
                            obj.key}'n.",
                        width=_WIDTH,
                    )
                )
        ...
        if is_same and "force" not in self.switches:
            string = wrap(
                f"|G{obj.name}|n already has the typeclass |G'{
                    new_typeclass
                }'|n. Use /force to override.",
                width=_WIDTH,
            )
        else:
            ...
            if not check_typeclass_perm(caller, new_typeclass):
                err = (
                    f"You don't have permission to use |G'{
                        new_typeclass}'|n typeclass."
                )
                caller.msg(wrap(err, width=_WIDTH))
                return

            if reset and "yes" not in self.switches:
                answer = yield (
                    wrap(
            ...
            if "prototype" in self.switches:
                ...
                prompt = (
                    f"Applying prototype |G'{prototype['key']}'|n over |G'{
                        obj.name
                    }'|n will cause the"
                    f" follow changes:|/|/|M{txt}|n"
                )
                if not reset:
                    prompt += (
                        "|/|/|RWARNING:|n Use the /reset switch to apply the prototype over a blank"
                        " state."
                    )
                prompt += "|/|/Are you sure you want to apply these changes [yes]/no?"

                if not "yes" in self.switches:
                    answer = yield (wrap_para(prompt, width=_WIDTH))
            ...
            if "prototype" in self.switches:
                ...
                if not prototype_success:
                    caller.msg(f"Prototype |G'{
                               prototype['key']}'|n failed to apply.")

            if is_same:
                string = f"|G'{obj.name}'|n updated its existing typeclass (|G'{
                    obj.path
                }'|n). "
            else:
                string = (
                    f"|G'{obj.name}'|n changed typeclass from |G'{
                        old_typeclass_path
                    }'|n to"
                    f" |G'{obj.typeclass_path}'|n. "
                )
            if update:
            ...
            if reset:
                ...
                string += (
                    " Attributes set before swap were not removed. (use |c`swap`|n or |c`type/reset`|n to"
                    " clear all)."
                )
            if "prototype" in self.switches and prototype_success:
                string += (
                    f" Prototype |G'{
                        prototype['key']
                    }'|n was successfully applied over the object"
                    " type."
                )
...
class CmdWipe(ObjManipCommand):
    ...
    key = "@wipe"
    locks = "cmd:perm(wipe) or perm(Beginner Builder)"
    help_category = "Building"
    ...
    def func(self):
        ...
        if not obj:
            return
        if (
            not obj.access(caller, "edit") and not obj.access(
                caller, "attrcreate")
        ) or not obj.access(caller, "control"):
            caller.msg(
                wrap(
                    f"|RPermissions Error:|n You need either |R'control'|n or |R'edit'|n and"
                    f" |R'attrcreate'|n permissions on |G'{
                        obj.name
                    }'|n to wipe its attributes",
                    width=_WIDTH,
                )
            )
            return
...
class CmdLock(ObjManipCommand):
    ...
    them by ';', i.e:
       'get:id(25); delete:perm(Beginner Builder)'
    """

    key = "@lock"
    aliases = ["@locks"]
    locks = "cmd: perm(locks) or perm(Beginner Builder)"
    help_category = "Building"
    ...
    def func(self):
        ...
        if "/" in self.lhs:
            ...
            if access_type == "control" and not has_control_access:
                ...
                caller.msg(
                    wrap(
                        "|RPermission Error:|n You need |R'control'|n access to change this type of lock.",
                        width=_WIDTH,
                    )
                )
                return
            ...
            if not has_control_access:
                caller.msg(
                    wrap(
                        f"|RPermission Error:|n You need |R'control'|n"
                        f" permissions on |G{obj}|n to change its locks!",
                        width=_WIDTH,
                    )
                )
                return
            ...
            if lockdef:
                ...
                string = f"|G{obj}|n has no lock of access type |R'{
                    access_type}'|n."
            caller.msg(wrap(string, width=_WIDTH))
            return
        ...
        if self.rhs:
            ...
            if self.switches:
                ...
                caller.msg(
                    wrap(
                        f"Switch(es) |C{swi}|n can not be used with a "
                        "lock assignment. Use e.g. "
                        "|Clock/del objname/locktype|n instead.",
                        width=_WIDTH,
                    )
                )
                return
            ...
            if not obj.access(caller, "control"):
                caller.msg(
                    wrap(
                        f"|RPermission Error:|n You need |R'control'|n"
                        f" permissions to edit Locks on |G'{obj}'|n.",
                        width=_WIDTH,
                    )
                )
                return
        ...
        if not obj.access(caller, "examine"):
            caller.msg(
                wrap(
                    f"|RPermission Error: You need |R'examine'|n"
                    f" permissions on {obj} to view its locks",
                    width=_WIDTH,
                )
            )
            return
...
class CmdExamine(ObjManipCommand):
    ...
    key = "@examine"
    aliases = ["@ex", "@exam"]
    locks = "cmd:perm(examine) or perm(Beginner Builder)"
    help_category = "Building"
    arg_regex = r"(/\w+?(\s|$))|\s|$"
    ...
    def msg(self, text):
        ...
        """
        evmore.msg(self.caller, text, exit_on_lastpage=True)

    def format_key(self, obj):
        return f"|G{obj.name}|n ({obj.dbref})"
    ...
    def format_typeclass(self, obj):
        if hasattr(obj, "typeclass_path"):
            return f"|G{obj.typename} ({obj.typeclass_path})|n"
    ...
    def format_account_typeclass(self, account):
        return f"|G{account.typename} ({account.typeclass_path})|n"
    ...
    def format_location(self, obj):
        if hasattr(obj, "location") and obj.location:
            return f"|M{obj.location.key}|n (#{obj.location.id})"

    def format_home(self, obj):
        if hasattr(obj, "home") and obj.home:
            return f"|M{obj.home.key}|n (#{obj.home.id})"

    def format_destination(self, obj):
        if hasattr(obj, "destination") and obj.destination:
            return f"|M{obj.destination.key}|n (#{obj.destination.id})"
    ...
    def format_scripts(self, obj):
        if (
            ...
            return f"|G{obj.scripts}|n"

    def format_single_tag(self, tag):
        if tag.db_category:
            return f"|B{tag.db_key}[{tag.db_category}]|n"
        else:
            return f"|B{tag.db_key}|n"
    ...
    def format_single_cmdset_options(self, cmdset):
        def _truefalse(string, value):
            if value is None:
                return ""
            if value:
                return f"|w{string}|n: T"
            return f"|w{string}|n: F"
    ...
    def format_single_cmdset(self, cmdset):
        ...
        return f"|Y{cmdset.path} [{cmdset.key}]|n ({cmdset.mergetype}, prio {cmdset.priority}{options})"
        ...
        if hasattr(obj, "cmdset"):
            ...
            for cmdset in stored_cmdsets:
                if cmdset.key != "_EMPTY_CMDSET":
                    stored_cmdset_strings.append(
                        self.format_single_cmdset(cmdset))
    ...
    def format_merged_cmdsets(self, obj, current_cmdset):
        ...
        all_cmdsets = [(cmdset.key, cmdset)
                       for cmdset in current_cmdset.merged_from]
    ...
    def format_attributes(self, obj):
        output = "\n  " + "\n  ".join(
            sorted(
                self.format_single_attribute(attr) for attr in obj.db_attributes.all()
            )
        )
        if output.strip():
    ...
    def format_exits(self, obj):
        if hasattr(obj, "exits"):
            exits = ", ".join(
                f"{exit.name}({exit.dbref})" for exit in obj.exits)
    ...
    def format_script_timer_data(self, obj):
        if hasattr(obj, "db_interval") and obj.db_interval > 0:
            ...
            active = (
                "|grunning|n" if obj.db_is_active and next_repeat else "|rinactive|n"
            )
    ...
    def format_channel_account_subs(self, obj):
        if hasattr(obj, "db_account_subscriptions"):
            ...
            if account_subs:
                return "\n  " + "\n  ".join(
                    format_grid(
                        [sub.key for sub in account_subs], sep=" ", width=_DEFAULT_WIDTH
                    )
                )
    ...
    def format_channel_object_subs(self, obj):
        if hasattr(obj, "db_object_subscriptions"):
            ...
            if object_subs:
                return "\n  " + "\n  ".join(
                    format_grid(
                        [sub.key for sub in object_subs], sep=" ", width=_DEFAULT_WIDTH
                    )
                )
    ...
    def get_formatted_obj_data(self, obj, current_cmdset):
        ...
        if inherits_from(obj, evennia.DefaultObject) and obj.has_account:
            ...
            objdata["  Account Typeclass"] = self.format_account_typeclass(
                obj.account)
            objdata["  Account Permissions"] = self.format_account_permissions(
                obj.account
            )
        ...
        if self.object_type == "channel":
            objdata["Subscription Totals"] = self.format_channel_sub_totals(
                obj)
            objdata["Account Subscriptions"] = self.format_channel_account_subs(
                obj)
            objdata["Object Subscriptions"] = self.format_channel_object_subs(
                obj)
    ...
    def format_output(self, obj, current_cmdset):
        ...
        for header, block in objdata.items():
            if block is not None:
                ...
                max_width = max(
                    max_width, max(display_len(line)
    ...
    def func(self):
        ...
        for obj, obj_attrs in self.examine_objs:
            ...
            if obj_attrs:
                ...
                attrs = [
                    attr for attr in obj.db_attributes.all() if attr.db_key in obj_attrs
                ]
                if not attrs:
...
class CmdFind(COMMAND_DEFAULT_CLASS):
    ...
    locks = "cmd:perm(find) or perm(Beginner Builder)"
    help_category = "Building"
    ...
    def func(self):
        ...
        if is_dbref or is_account:
            if is_dbref:
                ...
                string = "|wExact dbref match|n(#%i-#%i%s):" % (low,
                                                                high, restrictions)
            else:
                # an account search
            ...
            if "room" in switches:
                result = result if inherits_from(
                    result, ROOM_TYPECLASS) else None
            if "exit" in switches:
                result = result if inherits_from(
                    result, EXIT_TYPECLASS) else None
            if "char" in switches:
                result = result if inherits_from(
                    result, CHAR_TYPECLASS) else None
            ...
            if not result:
                ...
                string += (
                    f"\n   |RNo match found for '{
                        searchstring}' in #dbref interval.|n"
                )
            else:
                result = result[0]
                string += (
                    f"\n|g   {result.get_display_name(caller)}"
                    f"{result.get_extra_display_name_info(
                        caller)} - {result.path}|n"
                )
                if "loc" in self.switches and not is_account and result.location:
                    string += (
                        f" (|wlocation|n: |g{
                            result.location.get_display_name(caller)}"
                        f"{result.location.get_extra_display_name_info(
                            caller)}|n)"
                    )
        else:
            ...
            if "exact" in switches:
                keyquery = Q(db_key__iexact=searchstring,
                ...
                keyquery = Q(db_key__icontains=searchstring,
                             id__gte=low, id__lte=high)
                aliasquery = Q(
            ...
            if nresults:
                ...
                for res in results:
                    string += (
                        "\n  "
                        f" |g{res.get_display_name(caller)}|n"
                    ...
                    if (
                        ...
                        string += (
                            " (|wlocation|n:"
                            f" |G{res.location.get_display_name(caller)}|n"
...
class ScriptEvMore(evmore.EvMore):
    """
...
class CmdScripts(COMMAND_DEFAULT_CLASS):
    ...
    locks = "cmd:perm(scripts) or perm(Beginner Builder)"
    ...
    def _search_script(self):
        ...
        if dbref(self.typeclass_query):
            ...
            if scripts:
                return scripts
            self.caller.msg(
                wrap(f"No script found with dbref {
                     self.typeclass_query}", width=_WIDTH)
            )
            raise InterruptCommand
    ...
    def parse(self):
        ...
        if self.rhs:
            ...
            self.key_query, self.typeclass_query = _separate_key_typeclass(
                self.rhs)
    ...
    def func(self):
        ...
        if self.typeclass_query:
            scripts = self._search_script()
        else:
            scripts = None

        if self.obj_query:
            objects = caller.search(self.obj_query, quiet=True)
        else:
            objects = None
        ...
        if not self.switches:
            ...
            if obj:
                ...
                if self.rhs:
                    # creation mode
                    if check_typeclass_perm(
                        caller, self.typeclass_query
                    ) and obj.access(caller, "edit"):
                        if obj.scripts.add(
                            self.typeclass_query, key=self.key_query, autostart=True
                        ):
                            caller.msg(
                                wrap(
                                    f"Script |w{
                                        self.rhs}|n successfully added and"
                                    f" started on {
                                        obj.get_display_name(caller)}.",
                                    width=_WIDTH,
                                )
                            ...
                            caller.msg(
                                wrap(
                                    f"Script {
                                        self.rhs
                                    } could not be added and/or started "
                                    f"on {
                                        obj.get_display_name(caller)
                                    } (or it started and "
                        ...
                        caller.msg(
                            wrap(
                                f"|RPermissions Error:|n You don't have"
                                f" permission to create Script {self.rhs} with"
                                f" typeclass |G'{self.typeclass_query}'|n on"
                                f" |G{obj.get_display_name(caller)}|n.",
                                width=_WIDTH,
                            )
                        )
                else:
                    ...
                    if scripts:
                        ...
                        caller.msg(
                            wrap(f"No scripts defined on {obj}.", width=_WIDTH))
            elif scripts:
                # show found script(s)
                ScriptEvMore(caller, scripts.order_by(
                ...
                try:
                    if check_typeclass_perm(
                        caller, self.typeclass_query, check="global_script"
                    ):
                        ...
                        caller.msg(
                            wrap(
                                f"|RPermissions Error:|n You don't have permissions"
                                f" to create GLOBAL_SCRIPTS with typeclass"
                                f" |G{self.typeclass_query}|n.",
                                width=_WIDTH,
                            )
                        )
                        new_script = None
                ...
                if new_script:
                    caller.msg(
                        f"Global Script Created - {new_script.key} ({
                            new_script.typeclass_path
                        })"
                    )
                    ...
                    caller.msg(
                        wrap(
                            f"Global Script |rNOT|n Created - arguments: {
                                self.args}",
                            width=_WIDTH,
                        )
                    )
            ...
            if scripts.count() > 1:
                ret = yield (
                    wrap(
                        f"Multiple scripts found: {
                            scripts}. Are you sure you want to "
            ...
            for script in scripts:
                ...
                scripttype = f"Script on |G{obj}|n" if obj else "Global Script"
                for switch in self.switches:
                    verb = self.switch_mapping[switch]
                    if script.obj is not None:
                        change_ok = (
                            check_typeclass_perm(
                                caller, script, check="create")
                            and script.obj.access(caller, "edit")
                            and script.access(caller, "edit", default=True)
                        )
                        scripttype = f"Script on |G{script.obj}|n"
                    else:
                        scripttype = "GLOBAL_SCRIPT"
                        change_ok = check_typeclass_perm(
                            caller, script, check="global_script"
                        ) and script.access(caller, "edit", default=True)
                    if change_ok:
                        try:
                            ...
                            msgs.append(
                                wrap(
                                    f"{scripttype} {verb} - {script_key} ({
                                        script_typeclass_path
                                    })",
                                    width=_WIDTH,
                                )
                            )
                        ...
                        msgs.append(
                            wrap(
                                f"|RPermission Error:|n"
                                f" {scripttype} |Rnot|n {verb} - {script_key} ({
                                    script_typeclass_path
                                })",
                                width=_WIDTH,
                            )
...
class CmdObjects(COMMAND_DEFAULT_CLASS):
    ...
    key = "@objects"
    locks = "cmd:perm(listobjects) or perm(Beginner Builder)"
    help_category = "System"
        ...
        for obj in objs:
            latesttable.add_row(
                utils.datetime_format(
                    obj.date_created), obj.dbref, obj.key, obj.path
            )
...
class CmdTeleport(COMMAND_DEFAULT_CLASS):
    ...
    locks = "cmd:perm(teleport) or perm(Beginner Builder)"
    help_category = "Building"
    ...
    def func(self):
        ...
        if "tonone" in self.switches:
            ...
            if not obj_to_teleport.access(caller, "teleport"):
                err = (
                    f"|RPermission Error:|n You don't have permission to"
                    f" teleport |G{obj_to_teleport}|n."
                )
                caller.msg(wrap(err, width=_WIDTH))
                return
            if obj_to_teleport.has_account:
                caller.msg(
                    wrap(
                        f"Cannot teleport a puppeted object"
                        f" (|G{obj_to_teleport.key}|n, puppeted by"
                        f" |G{obj_to_teleport.account}|n) to a None-location.",
                        width=_WIDTH,
                    )
                )
                return
            ...
            if obj_to_teleport.location and "quiet" not in self.switches:
                obj_to_teleport.location.msg_contents(
                    wrap(
                        f"|G{caller}|n teleported |G{obj_to_teleport}|n"
                        f" into nothingness.",
                        width=_WIDTH,
                    ),
                    exclude=caller,
        ...
        if not destination:
            caller.msg(wrap("Destination not found.", width=_WIDTH))
            return

        if "loc" in self.switches:
            destination = destination.location
            if not destination:
                caller.msg(wrap("Destination has no location.", width=_WIDTH))
                return

        if obj_to_teleport == destination:
            caller.msg(
                wrap("You can't teleport an object inside of itself!", width=_WIDTH)
            )
            return
        ...
        if obj_to_teleport.location and obj_to_teleport.location == destination:
            caller.msg(
                wrap(
                    f"|G{obj_to_teleport}|n is already at |G{destination}|n.",
                    width=_WIDTH,
                )
            )
            return
        ...
        if not (
            ...
            caller.msg(
                wrap(
                    f"|RPermission Error:|n |G{obj_to_teleport}|n"
                    f" 'teleport'-lock blocks"
                    f" you from teleporting it anywhere.",
                    width=_WIDTH,
                )
            )
            return
        ...
        if not (
            ...
            caller.msg(
                wrap(
                    f"|RPermission Error:|n |G{
                        destination}|n 'teleport_here'-lock"
                    f" blocks |G{obj_to_teleport}|n from moving there.",
                    width=_WIDTH,
                )
            )
            return
        ...
        if not obj_to_teleport.location:
            ...
            caller.msg(
                wrap(
                    f"Teleported |G{
                        obj_to_teleport}|n None -> |G{destination}|n.",
                    width=_WIDTH,
                )
            )
            ...
            if obj_to_teleport == caller:
                caller.msg(wrap(f"Teleported to |G{
                           destination}|n.", width=_WIDTH))
            else:
                caller.msg(
                    wrap(
                        f"Teleported |G{
                            obj_to_teleport}|n -> |G{destination}|n.",
                        width=_WIDTH,
                    )
                )
        else:
            caller.msg(wrap("Teleportation failed.", width=_WIDTH))
...
class CmdTag(COMMAND_DEFAULT_CLASS):
    ...
    locks = "cmd:perm(tag) or perm(Beginner Builder)"
    help_category = "Building"
    arg_regex = r"(/\w+?(\s|$))|\s|$"
    ...
    def func(self):
        ...
        if "search" in self.switches:
            ...
            if nobjs > 0:
                ...
                matchstr = ", ".join(o.get_display_name(self.caller)
                                     for o in objs)
        ...
        if "del" in self.switches:
            ...
            if not (obj.access(caller, "control") or obj.access(caller, "edit")):
                err = wrap(
                    f"|RPermission Error:|n You need |R'edit'|n permissions on"
                    f" |G{obj}|n to delete its tags.",
                    width=_WIDTH,
                )
                caller.msg(err)
                return
            if self.rhs:
                ...
                category = None
                if ":" in tag:
                    tag, category = [part.strip()
                                     for part in tag.split(":", 1)]
                if obj.tags.get(tag, category=category):
                    obj.tags.remove(tag, category=category)
                    string = "Removed tag |B'%s'|n%s from |G%s|n." % (
                        tag,
                        " (category: %s)" % category if category else "",
                        obj,
                    )
                else:
                    string = "No tag |B'%s'|n%s to delete on |G%s|n." % (
                        tag,
                        " (category: %s)" % category if category else "",
                        obj,
                    )
        ...
        if self.rhs:
            ...
            string = "Added tag |B'%s'|n%s to |G%s|n." % (
                tag,
                " (category: %s)" % category if category else "",
                obj,
            )
            ...
            if not obj:
                return
            if not (obj.access(caller, "control") or obj.access(caller, "examine")):
                err = wrap(
                    f"|RPermission Error:|n You need |R'examine'|n permissions on"
                    f" |G{obj.key}|n to delete its tags.",
                    width=_WIDTH,
                )
                caller.msg(err)
                return
            ...
            if ntags:
                string = "Tag%s on |G%s|n: %s" % (
                    "s" if ntags > 1 else "",
                    obj,
                    ", ".join(
                        sorted(
                            "|B'%s'|n%s" % (tags[i], categories[i])
...
class CmdSpawn(COMMAND_DEFAULT_CLASS):
    ...
    Dictionary keys:
      |wprototype_parent  |n - name of parent prototype to use. Required if typeclass is
                               not set. Can be a path or a list for multiple inheritance
                               (inherits left to right). If set one of the parents must
                               have a typeclass.
    ...
    locks = "cmd:perm(spawn) or perm(Beginner Builder)"
    help_category = "Building"
    ...
    def _search_prototype(self, prototype_key, quiet=False):
        ...
        err = None
        if not prototypes:
            err = f"No prototype named |c'{prototype_key}'|n was found."
        elif nprots > 1:
            err = "Found {} prototypes matching '{}':\n  |c{}|n".format(
                nprots,
                prototype_key,
                ", ".join(proto.get("prototype_key", "")
            ...
            if not self.caller.locks.check_lockstring(
                ...
                prot = prototype
                err = (
                    f"|RPermission Error:|n You don't have access to use"
                    f" prototype |c{prot.pop('prototype_key')}|n."
                )
        if err:
    ...
    def _parse_prototype(self, inp, expect=dict):
        ...
        try:
            ...
            if not isinstance(prototype, expect):
                if eval_err:
                    ...
                    string = f"Expected |c{expect}|n, got |c{
                        type(prototype)}|n."
                self.msg(string)
                return
        ...
        if expect is dict:
            ...
            if "exec" in prototype and not self.caller.check_permstring("Developer"):
                self.msg(
                    "|Permission Error:|n |YSpawn aborted|n -"
                    " You are not allowed to use the 'exec' prototype key."
                )
                return
            try:
                ...
                self.msg(wrap(str(err), width=_WIDTH))
                return
        return prototype
    ...
    def _get_prototype_detail(self, query=None, prototypes=None):
        ...
        if prototypes:
            return ", ".join(protlib.prototype_to_str(prot) for prot in prototypes)
        elif query:
            self.msg(wrap(f"No prototype named |c'{
                     query}'|n was found.", width=_WIDTH))
        else:
            self.msg(wrap("No prototypes found.", width=_WIDTH))
    ...
    def list_prototypes(
        ...
        dbprot_query = []
        modprot_list = []
        uf_dbprot_query, uf_modprot_list = protlib.search_prototype(
            key, tags, return_iterators=True
        )
        prototypes = [
            protlib.search_prototype(prot, require_single=True)[0]
            if isinstance(prot, str)
            else prot
            for prot in uf_modprot_list
        ]
        ...
        for prot in prototypes:
            if not prot:
                continue
            protlib.validate_prototype(
                prot, None, protparents=None, is_prototype_base=True
            )
        for prototype in prototypes:
            ...
            # p = spawner._get_prototype(prot)
            if check_typeclass_perm(caller, prot.pop("typeclass")):
                modprot_list.append(prot)

        prototypes = [
            protlib.search_prototype(prot, require_single=True)[0]
            if isinstance(prot, str)
            else prot
            for prot in uf_dbprot_query
        ]
        prototypes = [protlib.homogenize_prototype(
            prot) for prot in prototypes]
        for prot in prototypes:
    ...
    def _list_prototypes(self, key=None, tags=None):
        ...
        self.list_prototypes(self.caller, key=key,
                             tags=tags, session=self.session)
    ...
    def _update_existing_objects(self, caller, prototype_key, quiet=False):
        ...
        if existing_objects:
            ...
            string = (
                f"There are {
                    n_existing} existing object(s) with an older version "
                f"of prototype '{prototype_key}'. Should it be re-applied to them{
                    slow
                }? [Y]/N"
            )
            answer = yield (string)
    ...
    def func(self):
        ...
        if (
            ...
            if self.lhs:
                ...
                # p = spawner._get_prototype(prot)
                if not check_typeclass_perm(caller, prot.pop("typeclass")):
                    err = (
                        f"|RPermission Error:|n You don't have typeclass"
                        f" permission |R'create'|n needed for editing"
                        f" |c'{prot.pop('prototype_key')}'|n."
                    )
                    caller.msg(wrap(err, width=_WIDTH))
                    return
            olc_menus.start_olc(
                caller, session=self.session, prototype=prototype)
            return
        ...
        if "raw" in self.switches:
            ...
            if not check_typeclass_perm(caller, prot.pop("typeclass")):
                err = (
                    f"|RPermission Error:|n You don't have typeclass"
                    f" permission |R'create'|n needed to view raw"
                    f" |c'{prot.pop('prototype_key')}'|n definition."
                )
                caller.msg(wrap(err, width=_WIDTH))
                return
        ...
        if "show" in self.switches or "examine" in self.switches:
            ...
            if not check_typeclass_perm(caller, prot.pop("typeclass")):
                err = (
                    f"|RPermission Error:|n You don't have typeclass"
                    f" permission |R'create'|n needed to examine"
                    f" |c'{prot.pop('prototype_key')}'|n definition."
                )
                caller.msg(wrap(err, width=_WIDTH))
                return
        ...
        if "save" in self.switches:
            ...
            if prototype_key is None:
                ...
                if not check_typeclass_perm(caller, prot.pop("typeclass")):
                    err = (
                        f"|RPermission Error:|n You don't have typeclass"
                        f" permission |R'create'|n needed to create the"
                        f" |c'{prot.pop('prototype_key')}'|n prototype."
                    )
                    caller.msg(wrap(err, width=_WIDTH))
                    return
            ...
            if prot_prototype_key != prototype_key:
                caller.msg(
                    wrap(
                        "(Replacing `prototype_key` in prototype with given key.)",
                        width=_WIDTH,
                    )
                )
            ...
            if prototype_desc is not None and prot_prototype_key != prototype_desc:
                caller.msg(
                    wrap(
                        "(Replacing `prototype_desc` in prototype with given desc.)",
                        width=_WIDTH,
                    )
                )
            ...
            diff = spawner.prototype_diff(
                old_prototype, prototype, homogenize=True)
            diffstr = spawner.format_diff(diff)
            new_prototype_detail = self._get_prototype_detail(prototypes=[
                                                              prototype])
            ...
            if answer.lower() in ["n", "no"]:
                ...
                return
            caller.msg(wrap("Saving prototype...\n\n", width=_WIDTH))
        ...
        if not self.args:
            ...
            caller.msg(
                wrap(
                    ...
                    f" - ({ncount} existing prototypes. Use /list to inspect)",
                    width=_WIDTH,
                )
            )
            return
        ...
        if "delete" in self.switches:
            ...
            if not check_typeclass_perm(caller, prot.pop("typeclass")):
                err = (
                    f"|RPermission Error:|n You don't have typeclass"
                    f" permission |R'create'|n needed to delete"
                    f" |c'{prot.pop('prototype_key')}'|n Prototype."
                )
                caller.msg(wrap(err, width=_WIDTH))
                return
            ...
            try:
                ...
                retmsg = f"|RError deleting:|n '{err}'."
            else:
                retmsg = (
                    "Deletion successful"
                    if success
            ...
            caller.msg(wrap(retmsg, width=_WIDTH))
            return
        ...
        if not check_typeclass_perm(caller, prot.pop("typeclass")):
            err = (
                f"|RPermission Error:|n You don't have typeclass"
                f" permission |R'create'|n needed to spawn"
                f" |c'{prot.pop('prototype_key')}'|n Prototype."
            )
            caller.msg(wrap(err, width=_WIDTH))
            return
...
# replace "building" with "builder_cmds" commands
cmds = [
    "CmdSetObjAlias",
    "CmdCopy",
    "CmdCpAttr",
...
def add_builder_cmds(cmd_set):
    """
    Adds new building commands
    """
    # Delete "building" commands
    for cmd in cmds:
        cmd_set.remove(eval(f"building.{cmd}()"))
    # Add "builder_cmds" commands
    for cmd in cmds:
        cmd_set.add(eval(f"{cmd}()"))

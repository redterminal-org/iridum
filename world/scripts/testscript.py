...
class TestScript(Script):
    ...
    * Hook methods (should also include self as the first argument):
    ...
    def at_script_creation(self):
        ...
        self.start(interval=100, start_delay=0, repeats=0)

    def at_server_reload(self):
        if self.db.time_to_next_repeat:
            self.update(interval=self.db.time_to_next_repeat)
            self.db.time_to_next_repeat = None
        else:
            self.db.time_to_next_repeat = self.time_until_next_repeat()
